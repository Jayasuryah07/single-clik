...
class ChatSentController extends GetxController {
  ...
  Future postChatApi() async {
    var bodyParams = {
      'enquiry_id': Get.arguments['enquiry_id'].toString(),
      'reply_id': Get.arguments['user_id'].toString()
    };
    debugPrint('bodyParams: $bodyParams');
    debugPrint('replyChat: ${API.replyChat}');
    log('token: ${await SharPreferences.getString(SharPreferences.token)}');
